...
import { GET_ALL_USERS, GET_ALL_CATEGORIES, GET_ALL_PRODUCTS, ADD_PRODUCT, ADD_CATEGORY, DELETE_USER, DELETE_CATEGORY, DELETE_PRODUCT, UPDATE_CATEGORY, GET_CATEGORY_DATA_TO_UPDATE, GET_PRODUCT_DATA_TO_UPDATE, UPDATE_PRODUCT, SET_IS_FETCHING, CREATE_TOKEN, GET_SINGLE_PRODUCT, ADD_COMMENT, GET_ALL_COMMENTS, SET_IS_FETCHING_MESSAGE } from "../constants/index";
...
const setIsFetchingMessage = (status) => {
  return {
...
export const addComment = (data) => async (dispatch) => {
  ...
  try {
    ...
    console.log("Error of addcomment:", error)
    // dispatch(setIsFetching(false));
  }
}

export const getAllComments = (userId, token) => async (dispatch) => {
  try {
    dispatch(setIsFetchingMessage(true));
    const res = await axios.get(`${process.env.REACT_APP_API_URL}comment`)
    console.log("Response for all messages:", res)
    if (res.status === 200) {
      dispatch({
    ...
    console.log("Error for all comments:")
    // dispatch(setIsFetchingTodos(false));
  }
